...
local e = None
local stop = True       -- 
local wustep = False -- controls per event stepping for waituntil construct; not user API
local condQ = {}
local allMonitors = {}
local allTallies = {}
...
---->        return message
    ...
    if type(etime) ~= "number" then
        error("Function simulate need a number parameter.")
    end
    endtime = etime
    message = "SimLua: Normal exit"
...
function Process.cancel(victim)
    -- Application function to cancel all event notices for this Process
    -- instance.
    e.unpost(victim)
end
